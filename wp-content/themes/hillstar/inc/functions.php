...
function hillstar_page_builder_components($components)
{
...
function hillstar_default_components($components)
{
...
function hillstar_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = hillstar_page_builder_components($components);
    } else {
        $components = hillstar_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'hillstar_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function hillstar_assets()
{
    return hillstar_theme()->getAssetsManager();
}
...
function hillstar_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'hillstar'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'hillstar'),
            'id' 			=> 'hillstar-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        hillstar_assets()
            ...
            ->registerStylesheet("hillstar-theme", "/theme/theme.css")
    ...
    hillstar_assets()->registerTemplateStyle('hillstar-theme-extras', '/theme/extras.css', 'hillstar-theme');
...
//blog options
    ...
    function hillstar_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'hillstar_show_post_meta_setting_filter');


    function hillstar_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'hillstar_posts_per_row_setting_filter');

    function hillstar_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'hillstar_archive_post_highlight_setting_filter');


    function hillstar_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'hillstar_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'hillstar_blog_sidebar_enabled_setting_filter' );

    function hillstar_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'hillstar_override_with_thumbnail_image');

    function hillstar_print_archive_entry_class($class = "")
    {
        ...
        $postsPerRow = max(1, apply_filters('hillstar_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('hillstar_archive_entry_class', $classes);
    ...
    function hillstar_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('hillstar_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'hillstar_get_started_info_page_tab');

    function hillstar_get_started_info_page_tab($tabs)
    {
    ...
    function hillstar_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'hillstar_theme_plugins');


    add_filter('http_request_host_is_external', 'hillstar_allow_internal_host', 10, 3);
    function hillstar_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_hillstar_front_set_predesign', function () {
        check_ajax_referer( 'hillstar_front_set_predesign_nonce', 'nonce' );
    ...
    function hillstar_override_main_row_class($classes)
    {
        return Defaults::get('templates.blog.row.layout-classes', $classes);
    }

    Hooks::prefixed_add_filter('main_row_class', 'hillstar_override_main_row_class', 10, 1);
